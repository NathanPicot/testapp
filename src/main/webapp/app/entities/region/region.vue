<template>
  <div>
    <h2 id="page-heading" data-cy="RegionHeading">
      <span v-text="t$('testappApp.region.home.title')" id="region-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('testappApp.region.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'RegionCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('testappApp.region.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && regions && regions.length === 0">
      <span v-text="t$('testappApp.region.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="regions">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('testappApp.region.regionName')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="testappApp.region.delete.question" data-cy="regionDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-region-heading" v-text="t$('testappApp.region.delete.question', { id: removeId })"></p>
      </div>
