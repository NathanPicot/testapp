<template>
  <div>
    <h2 id="page-heading" data-cy="LocationHeading">
      <span v-text="t$('testappApp.location.home.title')" id="location-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('testappApp.location.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'LocationCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="t$('testappApp.location.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && locations && locations.length === 0">
      <span v-text="t$('testappApp.location.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="locations">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('testappApp.location.streetAddress')"></span></th>
            <th scope="row"><span v-text="t$('testappApp.location.postalCode')"></span></th>
            <th scope="row"><span v-text="t$('testappApp.location.city')"></span></th>
            <th scope="row"><span v-text="t$('testappApp.location.stateProvince')"></span></th>
            <th scope="row"><span v-text="t$('testappApp.location.country')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="testappApp.location.delete.question" data-cy="locationDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-location-heading" v-text="t$('testappApp.location.delete.question', { id: removeId })"></p>
      </div>
