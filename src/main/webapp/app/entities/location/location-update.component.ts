      ...
      if (this.location.id) {
        this.locationService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showInfo(this.t$('testappApp.location.updated', { param: param.id }));
          })
          .catch(error => {
            this.isSaving = false;
        ...
        this.locationService()
          ...
          .then(param => {
            this.isSaving = false;
            this.previousState();
            this.alertService.showSuccess(this.t$('testappApp.location.created', { param: param.id }).toString());
          })
          .catch(error => {
            this.isSaving = false;
            this.alertService.showHttpError(error.response);
