<template>
  <div>
    <h2 id="page-heading" data-cy="TaskHeading">
      <span v-text="t$('testappApp.task.home.title')" id="task-heading"></span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="t$('testappApp.task.home.refreshListLabel')"></span>
        </button>
        <router-link :to="{ name: 'TaskCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-task">
            ...
            <span v-text="t$('testappApp.task.home.createLabel')"></span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tasks && tasks.length === 0">
      <span v-text="t$('testappApp.task.home.notFound')"></span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="tasks">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="t$('testappApp.task.title')"></span></th>
            <th scope="row"><span v-text="t$('testappApp.task.description')"></span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <template #modal-title>
        <span id="testappApp.task.delete.question" data-cy="taskDeleteDialogHeading" v-text="t$('entity.delete.title')"></span>
      </template>
      <div class="modal-body">
        <p id="jhi-delete-task-heading" v-text="t$('testappApp.task.delete.question', { id: removeId })"></p>
      </div>
